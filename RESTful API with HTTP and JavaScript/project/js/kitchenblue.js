  ...
  if (jsonObject.results.length == 0) { // Task 5: Part 1
    ...
    jsonObject.results.forEach(
      function (item, index) {
        ...
        if (thumbnail == "") {
          thumbnail = "images/generic_dish";
        } // Task 5, Part 3, display images/generic_dish.jpg if thumbnail is empty

        const href = item.href;

        formatedText += "<div class='dish-image-div'><a " + " href='" + href + "' target='_blank'><img class='dish-image' width='80' src='" + item.thumbnail + "' alt='recipe picture, link to recipe page'></a></div>";
